...
> 这是描述系统**当前实际形态**的唯一架构文档(2026-06-10 立,06-19 选稿核切 events 引擎,06-20 v1 落地;06-22 重磅度闸 + 降本增效,06-23 嵌入迁 Qwen3/MLX,06-24 语义问答上线 + GitHub 榜 v2,06-25 话题闸上线 + /eval 三考修 GitHub 榜 4 硬伤)。早期的 VPS/Jina API/即时推等设想已作废,本文只写现在跑的样子。
...
- **dedup** — 三级:URL规范化 → 内容指纹 → **本地 Qwen3-Embedding-0.6B/MLX**(1024维 L2归一,跑 Apple GPU/Metal,$0离线,`~/.cache/huggingface` 约633MB)语义近重复,阈值 **0.87 宁漏勿误**。**2026-06-23 从 jina-v3 迁来**(治 jina CPU 冷加载烤机卡死;`provider=mlx`,5 阈值重标,详见 `DESIGN.md`)。jina/fastembed 留回滚不删(`provider` 切回 `local`)。
...
- **render** — Jinja2 → 无头 Chrome(playwright)→ 两张「暖白+软黑+粉」便签风 PNG:详读长图 + 速读卡(见 `STYLE.md`)。
...
| embedding | **本地 Qwen3-Embedding-0.6B/MLX**(跑 Apple GPU,$0 离线;2026-06-23 从 jina-v3 迁来治烤机,`provider` 可切回 `local`/jina 回滚)。 |
